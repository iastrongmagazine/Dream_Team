...
def generate_dashboards(df, output_dir, num_cols, cat_cols):
    ...
    try:
        ...
        if not numeric_df.empty:
            ...
            stds = numeric_df.std()
            z_scores = np.abs(
                (numeric_df - numeric_df.mean()) / stds.replace(0, 1)
            ).fillna(0)
            ...
            bars = plt.bar(
                mean_abs_z.index,
                mean_abs_z,
                color=sns.color_palette("viridis", len(mean_abs_z)),
            )
            plt.axhline(
                1.0, color="red", linestyle="--", alpha=0.5, label="Typical Variance"
            )
...
if __name__ == "__main__":
    ...
    data_path = os.path.join(script_dir, "test_sales_data.csv")
