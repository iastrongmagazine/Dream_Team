...
def run_analysis(csv_path, output_dir):
    ...
    is_marketing = any(
        x in cols_lower for x in ["clicks", "campaign", "conversion", "impressions"]
    )
    is_health = any(
        x in cols_lower for x in ["patient", "diagnosis", "visit_date", "age"]
    )
    ...
    domain_name = (
        "Marketing"
        if is_marketing
        else ("Salud" if is_health else ("Ventas" if is_sales else "General"))
    )
    ...
    if is_marketing:
        primary_intensity = (
            "conversions"
            if "conversions" in cols_lower
            else (
                "clicks"
                if "clicks" in cols_lower
                else metric_cols[0]
                if metric_cols
                else None
            )
        ...
        primary_intensity = (
            "age" if "age" in cols_lower else (metric_cols[0] if metric_cols else None)
        )
    elif is_sales:
        primary_intensity = (
            "revenue"
            if "revenue" in cols_lower
            else (
                "profit"
    ...
    try:
        with open(peek_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"**Standard v4.2** | **Registros:** {len(df)} | **Intensidad Primaria:** `{primary_intensity}`\n\n"
            )
            ...
            f.write(
                f"- **Ratios Aplicados:** {', '.join(ratios.keys()) if ratios else 'Ninguno'}\n"
            )
            f.write(
                f"- **Segmentos:** {', '.join(cat_cols) if cat_cols else 'Global'}\n"
            )
    ...
    generate_dashboards(
        df, output_dir, metric_cols, cat_cols, primary_intensity, is_marketing
    )
    return df


def generate_dashboards(
    df, output_dir, num_cols, cat_cols, primary_intensity=None, is_marketing=False
):
    """
    ...
    try:
        if primary_intensity:
            ...
            save_plot(
                ...
                f"02. Success Density: {primary_intensity} Intensity",
            )
    except Exception as e:
    ...
    try:
        if cat_cols and primary_intensity:
            ...
            cat = cat_cols[0]
            plot_data = (
                df.groupby(cat)[primary_intensity]
                .sum()
        ...
        if cat_cols and num_cols:
            ...
            if is_marketing:
                ...
                target = (
                    ...
                    else (primary_intensity if primary_intensity else num_cols[0])
                )
            ...
            plt.ylabel(target)
            save_plot(
                "05_cat_vs_num_bar.png", f"05. Efficiency: {target} by {cat_cols[0]}"
            )
    except Exception as e:
    ...
    try:
        if cat_cols and primary_intensity:
            ...
            save_plot(
                ...
                f"08. Success Concentration ({primary_intensity} by {cat})",
            )
    except Exception as e:
    ...
    try:
        if cat_cols and primary_intensity:
            ...
            plot_data = (
                df.groupby(cat_cols[0])[primary_intensity]
                .sum()
                .sort_values(ascending=False)
            )
            plt.pie(
...
if __name__ == "__main__":
    ...
    data_path = os.path.join(script_dir, "marketing_campaigns.csv")
