...
sync_claude_config() {
    local claude_dir="$HOME/01_Core/03_Skills"
    ...
    if [ -d "$skills_dir" ]; then
        ...
        log_success "Synced all skills to ~/01_Core/03_Skills/"
    fi
}
...
show_menu() {
    ...
    echo ""
    echo "  ${CYAN}6)${NC} Sync to ~/01_Core/03_Skills/"
...
Options:
  ...
  --sync-claude Sync skills to ~/01_Core/03_Skills/
