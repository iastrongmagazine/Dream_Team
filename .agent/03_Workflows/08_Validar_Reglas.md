...
1. **Ingesta de Contexto (CRÍTICO)**
   ...
   - **Leer todas las reglas** en `.cursor/rules/`.
...
3. **Revisión de Reporte**
   ...
   - Si termina en **ROJO** (Error), revisar los logs para corregir:
     - Nombres de archivos en `04_Engine` (deben ser `NN_snake_case.py`).
     - Estructura de `.mdc` en `.cursor/rules` (campos `description`, `globs`).

## Integración con Ritual de Cierre

Este proceso se ejecuta automáticamente como parte de `04_Engine/08_Ritual_Cierre.py`.
